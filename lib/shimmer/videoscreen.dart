...
class VideoFeedScreens extends StatefulWidget {
  final String userId;

  const VideoFeedScreens({super.key, required this.userId});

  @override
  VideoFeedScreenState createState() => VideoFeedScreenState();
}

class VideoFeedScreenState extends State<VideoFeedScreens> with WidgetsBindingObserver {
  ...
  final Map<String, bool> _videoLikes = {}; // Track like state
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Container(
        ...
        child: StreamBuilder(
          stream: _firestore.collection('videos').where('userId', isEqualTo: widget.userId).orderBy('createdAt', descending: true).snapshots(),
              ...
              itemBuilder: (context, index) {
                ...
                return FutureBuilder<ChewieController>(
                  ...
                  builder: (context, chewieSnapshot) {
                    ...
                    _videoLikes[postId] ??= (videoDoc['likes'] as List).contains(_firebaseAuth.currentUser?.uid);
                    return Stack(
                      children: [
                        Container(
                          width: screenWidth,
                                ...
                                children: [
                                  FutureBuilder<Map<String, dynamic>>(
                                    ...
                                    builder: (context, userProfileSnapshot) {
                                      ...
                                      var profileImageUrl = userProfile['imageUrl'] ?? '';
                                            ...
                                            if (!isCurrentUser)
                                              ValueListenableBuilder<bool>(
                                                ...
                                                builder: (context, isFollowing, child) {
                                                  return GestureDetector(
                                                    onTap: () => _followUnfollowUser(userId),
                                                    child: Container(
                          ...
                          child: Column(
                            children: [
                              IconButton(
                                icon: Icon(
                                  size: 25,
                                  _videoLikes[postId] == true ? Icons.favorite : Icons.favorite_border,
                                  color: _videoLikes[postId] == true ? Colors.red : Colors.white,
                                ),
                                onPressed: () {
                                  _toggleLike(postId);
                                },
                                    ...
                                    builder: (context) =>
                                        DraggableScrollableSheet(
                                          ...
                                          builder: (context, scrollController) =>
                                              CommentSection(recipeId: postId),
                                        ),
                                  );
                                },
